...
from transcendence.utils.constants import (
...
class Profile(models.Model):
    ...
    def games_played(self):
        return self.user_games.count()

    @property
    def wins(self):
        return self.user_games.filter(is_winner=True).count()

    @property
    def losses(self):
        return self.user_games.filter(is_winner=False).count()

    @property
    def ratio(self):
        if self.losses == 0:
            return self.wins if self.wins > 0 else 0
        return round(self.wins / self.losses, 2)

    def get_friends(self):
        from .models import FriendInvitation

        friend_invitations = FriendInvitation.objects.filter(
    ...
    def get_sent_friend_invitations(self):
        ...
        sent_friend_invitations = FriendInvitation.objects.filter(
            models.Q(sender=self, status=STATUS_PENDING)
        )
    ...
    def get_received_friend_invitations(self):
        ...
        received_friend_invitations = FriendInvitation.objects.filter(
            models.Q(receiver=self, status=STATUS_PENDING)
        )
    ...
    def get_accepted_tournament_invitations(self):
        ...
        accepted_tournament_invitations = UserTournamentInvitation.objects.filter(
            user=self, status=STATUS_ACCEPTED
        )
    ...
    def get_received_tournament_invitations(self):
        ...
        received_tournament_invitations = UserTournamentInvitation.objects.filter(
            user=self, status=STATUS_PENDING
        )
...
class FriendInvitation(models.Model):
    sender = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="sent_friend_invitations"
    )
    receiver = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="received_friend_invitations"
    )
    ...
    def are_friends(cls, user, friend):
        is_friend = cls.objects.filter(
            ...
            & models.Q(receiver=friend)
            & models.Q(status=STATUS_ACCEPTED)
            | models.Q(sender=friend)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["sender", "receiver"], name="unique_friend_invitation"
            )
        ]
...
class Blocked(models.Model):
    blocker = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="blocker_blocked_users"
    )
    blocked = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="blocked_users"
    )
