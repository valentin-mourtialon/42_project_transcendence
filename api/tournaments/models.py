...
class Tournament(models.Model):
    TOURNAMENT_TYPE_CHOICES = [("REMOTE", "Remote"), ("LOCAL", "Local")]
    ...
    created_by = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="tournament_created"
    )
    ...
    def start(self, creator):
        ...
        # Get all the players
        players = list(
            UserTournamentInvitation.objects.filter(
                tournament=self, status=STATUS_ACCEPTED
            )
        )
    ...
    def get_invitations(self):
        return self.tournament_users.all()
...
class UserTournamentInvitation(models.Model):
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="user_tournaments"
    )
    tournament = models.ForeignKey(
        Tournament, on_delete=models.CASCADE, related_name="tournament_users"
    )
    status = models.CharField(
        max_length=50, choices=INVITATION_STATUS_CHOICES, default=STATUS_PENDING
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["user", "tournament"], name="unique_tournament_invitation"
            )
        ]
